...
class Command(BaseCommand):
        ...
        try:
            ...
            for index, row in sheets_data.iterrows():
                ...
                if best_match_row is not None:
                    ...
                    print(f"Desc: {row['Description']} not found in ERA strandard term")
                    handle_unmatched_data(row['Description'])
    ...
    def find_best_match(self, description, waste_category, volume, waste_stream_df, service_df, sub_service_df):
        ...
        desc_lower = description.lower() + ' ' + str(volume)
        ...
        if score_stream > 80 and (score_service > 80 or score_service_type > 30):
            matched_row_stream = waste_stream_df[waste_stream_df['stream_name'].apply(lambda x: x.lower()) == match_stream].iloc[0]

            # Finding service row based on matched service name and size
            service_name, service_size = match_service.split()[:-1], match_service.split()[-1]
